...
class Data_FromAudioFile(Dataset):
    
    def __init__(self, data_dir=DATA_DIR, train=True, transform=None):
        
        if train is True :
          self.data_dir = f'{data_dir}/train/train_audio'
          self.path_list = os.listdir(self.data_dir)
        else :
          self.data_dir = f'{data_dir}/test/test_audio'
          self.path_list = os.listdir(self.data_dir)
...
class Data_FromMFCCSFile(Dataset):
    
    def __init__(self, data_dir=DATA_DIR, train=True, transform=None):
        
        if train is True :
          self.data_dir = f'{data_dir}/train/train_mfccs'
          self.path_list = os.listdir(self.data_dir)
        else :
          self.data_dir = f'{data_dir}/test/test_mfccs'
          self.path_list = os.listdir(self.data_dir)
    ...
    def __getitem__(self, idx):
        ...
        mfccs = np.load(f'{self.data_dir}/{self.path_list[idx]}')
        label = self.path_list[idx][:2]
        tmp = np.zeros(3)
        tmp[int(DIC_LABEL_TO_VALUE[label])]=1
        label = tmp

        sample = {'audio': mfccs, 'label': label}
